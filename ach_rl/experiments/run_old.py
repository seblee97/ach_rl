...
def _organise_config_changes_and_checkpoint_dirs(
    ...
    for i, (run_name, changes) in enumerate(config_changes.items()):
        for j, seed in enumerate(seeds):
            ...
            if cluster:
                changes_copy.append({constants.GPU_ID: i * len(seeds) + j})
    ...
    if args.mode == constants.SINGLE:
        ...
        experiment_utils.config_changes_to_json(
            ...
            json_path=os.path.join(
                experiment_path, f"all_{constants.CONFIG_CHANGES_JSON}"
            ),
        )
        checkpoint_paths = _organise_config_changes_and_checkpoint_dirs(
            ...
            config_changes=config_changes,
            seeds=seeds,
            cluster=args.mode == constants.CLUSTER,
        )
        ...
        elif args.mode == constants.DISTRIBUTED_CLUSTER:
            if args.num_cpus:
                num_cpus = args.num_cpus
            else:
                raise ValueError(
        ...
        elif args.mode == constants.CLUSTER_ARRAY:
