...
class EnsembleQLearningRunner(base_runner.BaseRunner):
    ...
    def _train_episode(self, episode: int) -> Tuple[float, int]:
        ...
        self._write_scalar(
            tag=constants.Constants.MEAN_EPSILON,
            episode=episode,
            scalar=np.mean(ensemble_mean_epsilons)
        )
        self._write_scalar(
            tag=constants.Constants.MEAN_LR_SCALING,
            episode=episode,
            scalar=np.mean(ensemble_mean_lr_scalings)
        )
